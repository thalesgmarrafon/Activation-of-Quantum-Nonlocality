...
#fixed oeprators 'A1' and 'A2' for each independent Alice and fixed measurements for Bob
    if scenario == 'star':
        Is = (1/2**(n))*multikron(n, np.kron((A1 + A2), B1))
        Js = (1/2**(n))*multikron(n, np.kron((A1 - A2), B2))
    elif scenario == 'chain':
...
def meanG(rho, n, A1, A2, B1, B2, scenario): #Value of the inequality for the shared resource 'rho'
    if scenario == 'star':
        return (abs(np.trace(G(n, A1, A2, B1, B2, 'star')[0]@rho)))**(1/(n)) + (abs(np.trace(G(n, A1, A2, B1, B2, 'star')[1]@rho)))**(1/(n))
    elif scenario == 'chain':
        return (abs(np.trace(G(n, A1, A2, B1, B2, 'chain')[0]@rho)))**(1/(np.ceil((n+1)/2))) + (abs(np.trace(G(n, A1, A2, B1, B2, 'chain')[1]@rho)))**(1/(np.ceil((n+1)/2)))
    elif scenario == 'cyclic':
        return (abs(np.trace(G(n, A1, A2, B1, B2, 'cyclic')[0]@rho)))**(1/(np.ceil((n-1)/2))) + (abs(np.trace(G(n, A1, A2, B1, B2, 'cyclic')[1]@rho)))**(1/(np.ceil((n-1)/2)))
...
#in the specified 'scenario' with accuracy 'acc_new' for the method
    ...
    rho = multikron(n, state) #State shared in the network
...
def bis(n, scenario, ent, noise, acc_new = 1e-4, acc_bis = 1e-4): #Bissection method, 'n' parties in the network, specified 'scenario', 'ent1' and 'ent2'
...
#accuracy for parameter w calculated by bissection method and acc_new for newton method
    ...
    w1 = 1.0
    whalf = (w0 + w1)/2
    esthalf = whalf*ent + (1 - whalf)*noise
    S0 = -1.0
...
def w_nonlocal_network(n, scenario, ent, noise, acc_new = 1e-4, acc_bis = 1e-4):
    w = bis(n, scenario, ent, noise, acc_new = 1e-4, acc_bis = 1e-4)
